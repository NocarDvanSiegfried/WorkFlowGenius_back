...
def create_assignment():
    ...
    data = request.json
    if not data or 'task_id' not in data or 'assigned_to' not in data:
        return jsonify({
            'success': False,
    ...
    employee = User.query.get_or_404(assigned_to)
    
    if employee.role != 'employee':
        return jsonify({
            'success': False,
            'message': 'Можно назначать только сотрудникам'
        }), 400
    
    # Проверка существующего назначения
    existing = Assignment.query.filter_by(task_id=task_id, status='assigned').first()
    if existing:
        return jsonify({
            'success': False,
    ...
    employee.current_workload = min(
        employee.current_workload + workload_points,
...
def get_assignments():
    ...
    if user_id_filter and current_user.role != 'employee':
        query = query.filter_by(assigned_to=user_id_filter)
